...
class NyxRepository(context: Context, private val storage: StorageManager = StorageManager.getInstance(context.applicationContext)) {
    ...
    companion object {
        ...
        // ── 历史压缩参数 ─────────────────────────────────────────────────
        // 超过 COMPRESS_TRIGGER 条时自动触发；每次压缩最早的 COMPRESS_BATCH 条 → 1 条摘要
        const val COMPRESS_TRIGGER = 60
    ...
    // ── StorageManager ─────────────────────────────────────────────────────
    fun loadCharacters(): List<NyxCharacter>       = storage.load("characters", DEFAULT_CHARACTERS)
    fun saveCharacters(v: List<NyxCharacter>)      = storage.save("characters", v)
    fun loadWorldBook(): List<WorldBookEntry>       = storage.load("worldbook", DEFAULT_WORLD_BOOK)
    ...
    fun loadTtsConfig(): TtsConfig                  = storage.load("tts_config", TtsConfig())
    fun saveTtsConfig(v: TtsConfig)                 = storage.save("tts_config", v)
    ...
    suspend fun loadMemoriesDecayed(charId: String, limit: Int = 50): List<NyxMemory> = withContext(Dispatchers.IO) {
    ...
    suspend fun insertMemories(mems: List<NyxMemory>) = withContext(Dispatchers.IO) {
        ...
        mems.map { it.charId }.distinct().forEach { db.memoryDao().pruneForChar(it, 50) }
    }
    ...
    suspend fun shouldCompress(sessionId: String): Boolean = withContext(Dispatchers.IO) {
        ...
        (total - summaries) >= COMPRESS_TRIGGER
    }
...
角色：$reply
    ...
    private fun buildBody(model: String, messages: List<Map<String, String>>,
        ...
        JSONObject().apply {
            ...
            // · qwen3.x 系列（qwen3.5-flash / qwen3.7-max 等）：官方文档「默认开启思考」，必须关闭
            // · qwen-plus / qwen-flash / qwen-turbo / qwen-3.x：「默认不开启」，显式传 false 保险
