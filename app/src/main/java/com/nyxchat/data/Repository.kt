...
class NyxRepository(context: Context, private val storage: StorageManager = StorageManager.getInstance(context.applicationContext)) {
    ...
    // v6 fix: 新增 situationRules / deviationSignals 字段，旧 JSON 中不存在，
    // Gson 反序列化为 null，绕过 Kotlin 非空约束。在此统一兜底，防止：
    //   - PromptPipeline: isNotBlank() NPE
    //   - CharactersScreen: NyxField(value=null) NPE
    //   - PipelineOptimizer.estimateTokens(null) NPE
    fun loadCharacters(): List<NyxCharacter> =
        storage.load("characters", DEFAULT_CHARACTERS).map { c ->
            @Suppress("UNNECESSARY_SAFE_CALL")
            c.copy(
                replyLength      = c.replyLength ?: ReplyLength.Medium,
                situationRules   = c.situationRules   ?: "",
                deviationSignals = c.deviationSignals ?: ""
            )
        }
    ...
    fun loadApiConfig(): ApiConfig {
        ...
        return ApiConfig(
            apiKey       = key,
            baseUrl      = storage.loadSecure("api_base_url", "https://api.deepseek.com/v1"),
            model        = storage.loadSecure("api_model", "deepseek-v4-flash"),
            providerName = storage.loadSecureString("api_provider").ifBlank { "deepseek" },
    ...
    suspend fun extractMemories(cfg: ApiConfig, charId: String,
        ...
        // ── 结构化提取 prompt ────────────────────────────────────────────────
        // 改进：显式写出优先级规则和记忆类型，使 AI 的重要度赋值与衰减算法对齐。
        // - 承诺/约定（importance=5）→ Sensitive 类型，衰减权重 ×1.8，长期保留
        // - 重要个人信息（importance=4）→ Private 类型，仅归属角色可见
        // - 关系状态变化（importance=3-4）→ Public 类型
        // - 角色的主观推断（importance=2-3）→ HiddenInference 类型，注入时加「（我的猜测）」
        // 类型字段直接决定注入策略，必须精确。
        val prompt = """
从以下对话中提取值得长期记忆的信息，最多3条，只保留有实质意义的内容。

【优先级规则——决定 importance 分值，必须严格遵守】
★★★★★ importance=5（类型：Sensitive）
  → 承诺 / 约定（"我明天会…" / "你答应过…" / "我们说好了…" / 含时间节点的计划）
  → 用户说出的秘密、只对角色说的事

★★★★☆ importance=4（类型：Private）
  → 用户透露的重要个人信息（深层喜好、过往经历、身份、情感状态、家人/朋友关系）
  → 角色对用户做出的重要承诺或请求

★★★☆☆ importance=3（类型：Public）
  → 关系状态的明确变化（信任提升、冲突、和解、第一次表白、道歉）
  → 双方共同经历的重要时刻

★★☆☆☆ importance=2（类型：HiddenInference）
  → 角色对用户意图/状态的主观推断（"他似乎…" / "我感觉他…"）
  → 有参考价值但不确定的观察

【忽略】寒暄、重复内容、「好的」「嗯」之类的无实质信息回复、与已有记忆完全重复

【输出格式】每条独立一行，严格遵守以下格式：
[importance数字] [类型] 内容
示例：
[5] [Sensitive] 用户答应周末陪角色去看展览
[4] [Private] 用户的父亲最近生病住院，他很担心
[2] [HiddenInference] 用户提到"很累"时语气有些敷衍，可能在回避什么
如果没有值得提取的内容，只回复"无"。
...
角色：$reply
        """.trimIndent()
        val raw = callAI(cfg, listOf(mapOf("role" to "user", "content" to prompt)), 300, 0.3f)
        if (raw.trim() == "无" || raw.isBlank()) return emptyList()
        // 解析格式：[importance] [Type] 内容
        // 兼容 AI 可能输出的变体：[5][Sensitive]、[5] Sensitive 、[5][sensitive] 等
        val memLineRegex = Regex("""^\[(\d+)]\s*\[?(\w+)]?\s+(.+)$""")
        val validTypes = setOf("Public", "Private", "HiddenInference", "Sensitive")
        return raw.lines().mapNotNull { line ->
            ...
            // 类型容错：大小写不敏感匹配，未知类型按 importance 推断
            val rawType = match.groupValues[2].trim()
            val memType = validTypes.firstOrNull { it.equals(rawType, ignoreCase = true) }
                ?: when {
                    importance >= 5 -> "Sensitive"
                    importance == 4 -> "Private"
                    importance == 2 -> "HiddenInference"
                    else            -> "Public"
                }
            val content = match.groupValues[3].trim().ifBlank { return@mapNotNull null }
            NyxMemory(UUID.randomUUID().toString(), charId, content, importance,
                System.currentTimeMillis(), type = memType)
        }
    }
