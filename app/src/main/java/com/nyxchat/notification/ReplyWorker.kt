    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            val ctx = PipelineContext(
                ...
                // New fix: Worker 路径未传入 memoryInjectCount，Pipeline stage 一直用默认值 15，
                // 用户在 Settings 调整的记忆注入条数对通知回复路径完全无效。
            ...
            // B-1 fix: 原来硬编码 300，忽略角色级 replyLength 设置。
            // A-2 已保证 char.replyLength 非 null，此处可安全直接访问。
            val rawReply = repo.callAI(config, apiMsgs, char.replyLength.maxTokens, char.temperature)
